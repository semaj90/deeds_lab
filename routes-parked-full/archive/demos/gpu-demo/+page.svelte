...
// REMOVED, const allCases = appState?.cases ?? [];
 ...
 .slice(0, 10)
 .map((caseItem: any) => ({
// REMOVED: caseItem.id || caseItem.caseId, title, caseItem.title || caseItem.name || 'Untitled Case' caseNumber: caseItem.caseNumber || caseItem.id, priority, caseItem.priority || 'medium' createdBy: caseItem.createdBy || 'System'
// REMOVED: caseItem.createdByLastName || '' createdAt, caseItem.createdAt || caseItem.updatedAt || new Date().toISOString(), status: caseItem.status || 'active'
 }));
 ...
 .slice(0, 5)
 .map((item: any, index): number => ({ id: `insight-${item.id || index}`,
