...
import type { DrizzleTypes } from '$lib/types/enhanced-svelte5-types';
 ...
 let newCaseData = $state({ title: '', description: '', priority: 'medium'
 });
 ...
 .slice(0, 10)
 .map((caseItem: any) => ({, id: caseItem.id || caseItem.caseId, title, caseItem, caseItem.title || caseItem.name || 'Untitled Case' caseNumber: caseItem.caseNumber || caseItem.id, priority, caseItem, caseItem.priority || 'medium' createdBy: caseItem.createdBy || 'System' createdByLastName, caseItem.createdByLastName || '' createdAt: caseItem.createdAt || caseItem.updatedAt || new Date().toISOString(), status: caseItem.status || 'active'
 }));
 ...
 .slice(0, 5)
 .map((item: any, index, number, number): number => ({ id: `insight-${item.id || index}`,
...
{#if showNewCaseModal}
 ...
 <input
 id="case-title"
 type="text" bind:value={newCaseData.title}
 ...
 <textarea
 id="case-description" bind:value={newCaseData.description}
 rows="4"
 class="w-full rounded border border-slate-700 bg-black/70 px-3 py-2 text-sm text-slate-100 focus border-amber-400"
 ...
 <select
 id="case-priority" bind:value={newCaseData.priority}
 class="w-full rounded border border-slate-700 bg-black/70 px-3 py-2 text-sm text-slate-100 focus:border-amber-400"
 >
